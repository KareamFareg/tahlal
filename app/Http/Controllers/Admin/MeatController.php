...
class MeatController extends AdminController
{
    ...
    public function getAll($id){
        $products =  SouqProducts::where('shop_id' , 0)->where('category', 47)->get();
        if($products){
            return view('admin.meat.index',['products'=> $products]);
         }else{
     ...
     public function edit($local ,$id){
         ...
         if($product){
            return view('admin.meat.edit',['product'=> $product]);
         }else{
             $this->flashAlert(['error' => ['msg' => "عفوا لا توجد منتجات متاحه حاليا في هذا المتجر"]]);
              return back();
         }
    ...
    //طلبات الذبائح
   public function getAllMeatOrders(Request $request){
    $status = $request->route('type');
    if ($status == 6) {
        $orders = Order::with(['items', 'offer', 'shipper_data', 'user_data'])->where('type', 47)->orderBy('id', 'DESC');
    } else {
        $orders = Order::with(['items', 'offer', 'shipper_data', 'user_data'])->where('type', 47)->where('status', $status)->orderBy('id', 'DESC');
    }

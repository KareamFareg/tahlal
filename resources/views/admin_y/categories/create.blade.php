        ...
        <div class="kt-portlet__head">
          <div class="kt-portlet__head-label">
            <h3 class="kt-portlet__head-title">
              {{ __('words.add') }} &nbsp;&nbsp;&nbsp; <x-buttons.but_back link="{{ route('admin.categories.index') }}"/>
            </h3>
          </div>
        </div>
...
<!-- form -->
<form class="kt_form_1" enctype="multipart/form-data" action="{{route('admin.categories.store')}}" method="post">
    {{ csrf_field() }}
    ...
    <div class="form-group row">
      ...
      <div class="col-lg-4 col-md-9 col-sm-12">
        <select class="form-control kt-select2 {{ $errors->has('parents') ? ' is-invalid' : '' }}" id="kt_select2_3" name="parents">
          <option {{ old('parents') == 0 ? 'selected' : '' }} value="0">{{ __('category.main_category') }}</option>
          @foreach ( $parents as $parent )
            <option {{ old('parents') == $parent->id ? 'selected' : '' }} value="{{ $parent->id }}"> {{ $parent->title }} {{str_repeat('__', $parent->depth)}}</option>
          @endforeach
        </select>
        <!-- <span class="form-text text-muted">Please enter your full name</span> -->
    ...
    <div class="form-group row">
      ...
      <div class=" col-lg-4 col-md-9 col-sm-12">
        <input class="form-control {{ $errors->has('sort') ? ' is-invalid' : '' }}" type="number" min="1" value="1" maxlength="3" id="example-number-input" name="sort">
        <!-- <span class="form-text text-muted">Please enter your full name</span> -->
        @if ($errors->has('sort'))
    ...
    <div class="form-group row">
      ...
      <div class=" col-lg-6 col-md-9 col-sm-12">
        <textarea class="form-control {{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" maxlength="1000" placeholder="" rows="8"></textarea>
        <!-- <span class="form-text text-muted">Please enter your full name</span> -->

        ...
        <div class="kt-portlet__head">
          <div class="kt-portlet__head-label">
            <h3 class="kt-portlet__head-title">
              ...
              <x-admin.trans-bar :languages="$languages" route="{{ route('admin.countries.edit' , ['id' => $data->id ] ) }}" :trans="$trans"/>
              </div>
            </h3>
          </div>
        </div>
...
<form class="kt_form_1" enctype="multipart/form-data"
    @if ($data->country_info->isEmpty())
      action="{{route('admin.countries.store_trans',[ 'id'=> $data->id ]) }}" method="post"
    @else
      action="{{route('admin.countries.update' , ['id' => $data->country_info->first()->id ])}}"  method="post"
    @endif>
    ...
    <div class="form-group row">
      ...
      <div class=" col-lg-4 col-md-9 col-sm-12">
        ...
        value="{{ old('title' , optional($data->country_info->first())->title) }}" name="title" placeholder="">
        @if ($errors->has('title'))
      ...
      <div class="col-lg-4 col-md-9 col-sm-12">
        <select class="form-control kt-select2 {{ $errors->has('parent_id') ? ' is-invalid' : '' }}" id="kt_select2_3" name="parent_id">
          ...
          @foreach ( $countries as $country )
            <option {{ old('parent_id',$data->parent_id) == $country->id ? 'selected' : '' }} value="{{ $country->id }}"> {{ $country->title }} {{str_repeat('__', $country->depth)}}</option>
          @endforeach
        </select>

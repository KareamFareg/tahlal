...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
  ...
  <div class="kt-portlet">
    ...
    <!--begin::Form-->
    <form class="kt-form kt-form--label-right" action="{{ route('admin.subscriptionpackages.index') }}" method="post">
          {{ csrf_field() }}
      ...
      <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
        <thead>
          <tr>
            ...
            <th>{{ __('category.root') }}</th>
            <th>{{ __('subscription_package.for_clients') }}</th>
        ...
        <tbody>
          ...
          <tr id="{{ $item->id }}">
            <td>{{ $item->id }}</td>
            <td>
            <a href="{{ route('admin.subscriptionpackages.edit' , [ 'id' => $item->id ] ) }}" class="kt-userpic kt-userpic--circle kt-margin-r-5 kt-margin-t-5" data-toggle="kt-tooltip" data-placement="right">
              {{ $item->title }}
            </a>
            </td>
            <td>{{ optional($item->category->category_info->first())->title }}</td>
            <td>
            ...
            <td>{{ $item->price }}</td>
            <td>
              <form action="{{ route('admin.subscriptionpackages.status',['id' => $item->id ]) }}" onsubmit="ajaxForm(event,this,'dt_dv','er_dv','');"  enctype="multipart/form-data" method="post">
                {{ csrf_field() }}

...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
    <div class="kt-portlet kt-portlet--mobile">
        ...
        <div class="kt-portlet__body">
            <form class="kt_form_1 row" enctype="multipart/form-data" action="{{ route('admin.commission.not_paid') }}"
                method="post">
                {{ csrf_field() }}
                <div class=" form-group col-lg-3">
                    ...
                    <select class="form-control kt-select2 " id="kt_select2_3" name="shipper">
                        ...
                        <option {{ old('parents') == $user->id ? 'selected' : '' }} value="{{ $user->id }}">
                            {{ $user->name }}
                        </option>
                        @endforeach
                    </select> 
                ...
                {{-- <div class="form-group col-lg-1">
                    <label for="">&nbsp;</label>
                    <a  class="form-control btn btn-success " href="{{ route('admin.commission.index') }}"><i class="flaticon flaticon2-refresh"></i></a>
                </div> --}}
            </form>
        </div>
    </div>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $shipper->id }}</td>
                        <td>{{ $shipper->name }}</td>
                        <td>{{ $shipper->orders_price($ordersIds, 0) }}</td>
                        <td>{{ $shipper->shipping_price($ordersIds, 0) }}</td>
                        {{-- <td>{{ $shipper->shipper_amount($ordersIds, 0) }}</td> --}}
                        <td>{{ $shipper->commission($ordersIds, 0) }} ريال  </td>
                        <td>{{ $shipper->discount($ordersIds, 0) }} ريال  </td>
                        <td>{{ $shipper->payment($ordersIds,1, 0) }} ريال  </td>
                        <td>{{ $shipper->payment($ordersIds,2, 0) }} ريال  </td>
                        <td>{{ $shipper->payment($ordersIds,3, 0) }} ريال  </td>
                        <td>{{ $shipper->deserved_amount($ordersIds, 0) }} ريال  </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
